...
function generateMap(cfg, byName, provIndex) {
  ...
  let H;
  let project;
  let mapArea; // the drawn map rectangle (ocean) inside the parchment frame
  if (cfg.crop) {
    // Territories are clipped to `crop`; the viewBox adds `pad` degrees of
    // parchment border around it (the scroll edge), with ocean/land on top.
    const pad = cfg.pad ?? 0;
    ...
    mapArea = {
      x: Math.round(pad * s),
      y: Math.round(pad * s),
      width: Math.round((cfg.crop.lngMax - cfg.crop.lngMin) * s),
      height: Math.round((cfg.crop.latMax - cfg.crop.latMin) * s),
    };
  } else {
    H = 500;
  ...
  const map = { id: cfg.id, name: cfg.name, viewBox: `0 0 ${W} ${H}`, mapArea, regions: regionsOut, territories, connectors };
  if (decorations.length) map.decorations = decorations;

  // Backdrop: all world landmasses projected into this view, clipped to the
  // padded viewBox bounds, so non-playable land (beyond the crop) shows as
  // a dull shape rather than blue ocean when panning near the edges.
  if (cfg.crop) {
    const pad = cfg.pad ?? 0;
    const viewClip = {
      lngMin: cfg.crop.lngMin - pad,
      lngMax: cfg.crop.lngMax + pad,
      latMin: cfg.crop.latMin - pad,
      latMax: cfg.crop.latMax + pad,
    };
    const bdParts = [];
    for (const feature of byName.values()) {
      for (const ring of ringsOf(feature.geometry)) {
        const clipped = clipToBox(ring, viewClip);
        if (clipped.length < 3) continue;
        const projected = clipped.map(project);
        const simplified = simplify(projected, 3.0);
        if (simplified.length < 3) continue;
        bdParts.push("M" + simplified.map(([x, y]) => `${round(x)} ${round(y)}`).join("L") + "Z");
      }
    }
    if (bdParts.length) map.backdrop = bdParts.join("");
  }
...
const AFRICA = {
  ...
  spec: [
    ...
    { id: "senegal", name: "Senegal", region: "west-africa", countries: ["Senegal", "Gambia"] },
    { id: "guinea", name: "Guinea", region: "west-africa", countries: ["Guinea", "Guinea-Bissau"] },
    { id: "sierra-leone", name: "Sierra Leone & Liberia", region: "west-africa", countries: ["Sierra Leone", "Liberia"] },
    { id: "mali", name: "Mali", region: "west-africa", countries: ["Mali"] },
    { id: "ivory-coast", name: "Ivory Coast", region: "west-africa", countries: ["Ivory Coast"] },
    { id: "ghana", name: "Gold Coast", region: "west-africa", countries: ["Ghana"] },
    { id: "burkina-faso", name: "Upper Volta", region: "west-africa", countries: ["Burkina Faso"] },
    { id: "niger", name: "Niger", region: "west-africa", countries: ["Niger"] },
    { id: "togo-benin", name: "Togo & Dahomey", region: "west-africa", countries: ["Togo", "Benin"] },
    { id: "nigeria", name: "Nigeria", region: "west-africa", countries: ["Nigeria"] },
    { id: "chad", name: "Chad", region: "west-africa", countries: ["Chad"] },
    // Central Africa
    { id: "cameroon", name: "Cameroon", region: "central-africa", countries: ["Cameroon"] },
    { id: "gabon", name: "Gabon", region: "central-africa", countries: ["Gabon", "Equatorial Guinea"] },
    { id: "congo-brazzaville", name: "Congo", region: "central-africa", countries: ["Republic of the Congo"] },
    { id: "central-african-rep", name: "Ubangi-Shari", region: "central-africa", countries: ["Central African Republic"] },
    { id: "congo", name: "Congo Free State", region: "central-africa", countries: ["Democratic Republic of the Congo"] },
    { id: "angola", name: "Angola", region: "central-africa", countries: ["Angola"] },
    // East Africa
    { id: "sudan", name: "Sudan", region: "east-africa", countries: ["Sudan"] },
    { id: "south-sudan", name: "Equatoria", region: "east-africa", countries: ["South Sudan"] },
    { id: "ethiopia", name: "Abyssinia", region: "east-africa", countries: ["Ethiopia"] },
    { id: "eritrea", name: "Eritrea", region: "east-africa", countries: ["Eritrea", "Djibouti"] },
    { id: "somalia", name: "Somaliland", region: "east-africa", countries: ["Somalia", "Somaliland"] },
    { id: "uganda", name: "Uganda", region: "east-africa", countries: ["Uganda"] },
    { id: "kenya", name: "Kenya", region: "east-africa", countries: ["Kenya"] },
    { id: "rwanda-burundi", name: "Ruanda-Urundi", region: "east-africa", countries: ["Rwanda", "Burundi"] },
    { id: "tanzania", name: "Tanganyika", region: "east-africa", countries: ["United Republic of Tanzania"] },
    // Southern Africa
    { id: "zambia", name: "Northern Rhodesia", region: "southern-africa", countries: ["Zambia", "Malawi"] },
    { id: "zimbabwe", name: "Southern Rhodesia", region: "southern-africa", countries: ["Zimbabwe"] },
    { id: "mozambique", name: "Mozambique", region: "southern-africa", countries: ["Mozambique"] },
    { id: "namibia", name: "South-West Africa", region: "southern-africa", countries: ["Namibia"] },
    { id: "botswana", name: "Bechuanaland", region: "southern-africa", countries: ["Botswana"] },
    { id: "cape", name: "Cape Colony", region: "southern-africa", admin: "South Africa", countries: ["Lesotho"], provinces: ["Eastern Cape", "Western Cape", "Northern Cape", "KwaZulu-Natal"] },
    { id: "boer-republics", name: "Boer Republics", region: "southern-africa", admin: "South Africa", countries: ["eSwatini"], provinces: ["Free State", "Gauteng", "Limpopo", "Mpumalanga", "North West"] },
